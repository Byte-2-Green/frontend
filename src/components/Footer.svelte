<nav class="relative bg-white h-20 flex justify-around items-center shadow-md">
 <!-- Navigation items -->
  <!--Icons provided by icons8.com-->

  <section class="flex w-full justify-between px-6">
    <!-- Statistics -->
    <button class="flex flex-col items-center">
      <img width="30" height="30" src="https://img.icons8.com/ios/50/rebalance-portfolio.png" alt="rebalance-portfolio"/>
      <span class="text-sm font-medium text-black">Statistics</span>
    </button>

    <!-- Challenges -->
    <button class="flex flex-col items-center">
      <img width="30" height="30" src="https://img.icons8.com/ios/50/mission-of-a-company.png" alt="mission-of-a-company"/>
      <span class="text-sm font-medium text-black">Challenges</span>
    </button>

    <!-- Shop -->
    <button class="flex flex-col items-center">
      <img width="30" height="30" src="https://img.icons8.com/ios/50/shop--v1.png" alt="shop--v1"/>
      <span class="text-sm font-medium text-black">Shop</span>
    </button>

    <!-- Profile -->
    <button class="flex flex-col items-center">
      <img width="30" height="30" src="https://img.icons8.com/fluency-systems-regular/96/user--v1.png" alt="user--v1"/>
      <span class="text-sm font-medium text-black">Profile</span>
    </button>
  </section>

  <!-- Center home button -->
  <button class="absolute -top-8 mx-auto w-16 h-16 rounded-full bg-secondary-light flex items-center justify-center">
    <img width="150" height="150" src="https://img.icons8.com/bubbles/150/museum.png" alt="museum"/>
  </button>
</nav>
