...
function testServer() {
  http.get('http://localhost:5173', (response) => {
    ...
    if (statusCode === 200) {
      ...
      console.error(`Test failed: Server responded with status code ${statusCode}`);
    }
  }).on('error', (err) => {
    console.error(`Test failed: Could not connect to the server. Error: ${err.message}`);
  });
}
...
async function testApi() {
    try {
        ...
        if (response.status === 200) {
          ...
          console.error(`Test failed: Server responded with status code ${response.status}`);
        }
      } catch (err) {
        console.error(`Test failed: Could not connect to the server. Error: ${err.message}`);
      }
